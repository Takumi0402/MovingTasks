...
export function InventoryTable({ data, selectedCategoryKey, focusedNodeKey, onRowClick }: InventoryTableProps) {
    ...
    const updatePointStorage = useAppStore((state) => state.updatePointStorage);
        ...
        <table className="data-table inventory-table">
            ...
            <tbody>
                {allPoints.map((point) => {
                    ...
                    const isDemandForSelectedCategory = toAmount > fromAmount;
                    return (
                        <tr
                            key={point.key}
                            ref={(el) => {
                        ...
                        >
                            ...
                            <td>
                                <input
                                    type="checkbox"
                                    checked={point.storage}
                                    onChange={(e) => updatePointStorage(point.key, e.target.checked)}
                                    onClick={(e) => e.stopPropagation()}
                                    title={isDemandForSelectedCategory ? "この地点を保管庫属性に設定" : "現在のカテゴリでは需要ノードではありません（属性自体は保持されます）"}
                                />
                            </td>
                        </tr>
                    );
