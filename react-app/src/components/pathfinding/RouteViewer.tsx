...
export const RouteViewer = () => {
    ...
    // 3. ミニマップで選択された始点・終点
    const [mapStartNode, setMapStartNode] = useState<string | null>(null);
    const [mapEndNode, setMapEndNode] = useState<string | null>(null);

    const routeFromMapSelection = useMemo(() => {
        if (!data || !mapStartNode || !mapEndNode) return null;
        return data.routes.get(`${mapStartNode}_${mapEndNode}`) ?? null;
    }, [data, mapEndNode, mapStartNode]);

    const highlightedRoute = selectedRoute ?? routeFromMapSelection;
        ...
        <div style={{ display: "flex", gap: "20px", height: "100%" }}>
            ...
            <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column" }}>
                <RouteTablePanel
                    ...
                    onStartNodeChange={(nodeKey) => {
                        ...
                        setMapStartNode(nodeKey || null);
                        setMapEndNode(null);
                    }}
                    selectedRoute={selectedRoute}
                    onRouteSelect={(route) => {
                        setSelectedRoute(route);
                        setMapStartNode(route.from);
                        setMapEndNode(route.to);
                    }}
                />
            </div>
            ...
            <div style={{ flex: 2.3, minWidth: 0, border: "1px solid #ccc", borderRadius: "8px" }}>
                <RouteMiniMap
                    ...
                    selectedStartNodeKey={mapStartNode}
                    selectedEndNodeKey={mapEndNode}
                    onNodePairChange={(startNodeKey, endNodeKey) => {
                        setMapStartNode(startNodeKey);
                        setMapEndNode(endNodeKey);
                        setSelectedStartNode(startNodeKey ?? "");
                        if (!startNodeKey || !endNodeKey) {
                            setSelectedRoute(null);
                            return;
                        }

                        const route = data.routes.get(`${startNodeKey}_${endNodeKey}`) ?? null;
                        setSelectedRoute(route);
                    }}
                />
            </div>
