...
export function useExportTasksToExcel() {
    const exportToExcel = (solverResult: SolverResultByCategory[], objectCategories: Map<string, ObjectCategory>, penalty: number, timeLimit: number, selectedCategoryKeys: string[], zipFileName: string = "") => {
        ...
        solverResult.forEach((result) => {
            ...
            const fromSheetData = [["from", "to", "amount", "cost"], ...fromSorted.map((task) => [task.supplyNode, task.demandNode, task.amount, task.cost ?? ""])];
            sheets[`${baseName}(from整列)`] = XLSX.utils.aoa_to_sheet(fromSheetData);
            ...
            const toSheetData = [["from", "to", "amount", "cost"], ...toSorted.map((task) => [task.supplyNode, task.demandNode, task.amount, task.cost ?? ""])];
            sheets[`${baseName}(to整列)`] = XLSX.utils.aoa_to_sheet(toSheetData);
        });
