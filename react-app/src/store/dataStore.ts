...
interface AppState {
    ...
    updateNode: (key: string, newAreaKey: string, newName?: string, storage?: boolean) => void;
    ...
    updatePointStorage: (pointKey: string, storage: boolean) => void;
    isSolving: boolean;
    ...
    return {
        ...
        addNode: (type, key, areaKey, name) =>
            set((state) => {
                ...
                if (type === "point") {
                    const newPoints = new Map(state.data.points);
                    newPoints.set(key, new Point(key, name || key, areaKey, 0, 0, new Map(), false));
        ...
        updateNode: (key, newAreaKey, newName, storage) =>
            set((state) => {
                if (!state.data) return {};
        ...
        updatePointObjectQuantity: (pointKey, categoryKey, from, to) =>
            set((state) => {
                ...
                const newPoint = new Point(targetPoint.key, targetPoint.name, targetPoint.areaKey, targetPoint.x, targetPoint.y, newObjects, targetPoint.storage);
                newPoints.set(pointKey, newPoint);

                return { data: state.data.withNewPoints(newPoints) };
            }),

        updatePointStorage: (pointKey, storage) =>
            set((state) => {
                if (!state.data) return {};
                const newPoints = new Map(state.data.points);
                const targetPoint = newPoints.get(pointKey);
                ...
                const newPoint = new Point(targetPoint.key, targetPoint.name, targetPoint.areaKey, targetPoint.x, targetPoint.y, new Map(targetPoint.objects), storage);
                newPoints.set(pointKey, newPoint);
